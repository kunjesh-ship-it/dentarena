...
export function HomeHero() {
  ...
  return (
    <section className="relative overflow-hidden bg-background band-xl pt-12 md:pt-20 theme-color-1">
      <div
        aria-hidden="true"
        ...
        <Reveal isStaggerParent>
          <ul className="grid gap-x-8 gap-y-5 sm:grid-cols-2 lg:grid-cols-4">
            {trustPoints.map((point) => (
              <Reveal as="li" key={point} className="flex items-start gap-3 text-sm leading-relaxed text-ink">
...
export function HomeBeforeAfter() {
  ...
  // activeIndex is always clamped to cases.length so this is always defined
  ...
  const goPrev = useCallback(() => setActiveIndex((i) => (i - 1 + cases.length) % cases.length), [cases.length]);
  const goNext = useCallback(() => setActiveIndex((i) => (i + 1) % cases.length), [cases.length]);

  return (
    <section
      aria-labelledby="before-after-heading"
      className="relative overflow-hidden bg-[#0A1D24] band-lg text-white"
    >
      ...
      <Container className="relative">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <Reveal delay={60}>
            <h2 id="before-after-heading" className="mt-4 display-2 text-white">
        ...
        <Reveal delay={180} className="mx-auto mt-10 max-w-4xl md:mt-14">
          <div className="flex items-center gap-3 md:gap-5">
            ...
            {/* Slider card */}
            <div className="min-w-0 flex-1 rounded-3xl bg-white/[0.04] p-3 md:p-5 ring-1 ring-white/10 shadow-2xl backdrop-blur-sm">
              <div ref={containerRef} className="relative">
                <div
                  ...
                  className="group relative aspect-[3/2] w-full cursor-ew-resize overflow-hidden rounded-2xl select-none touch-none focus:outline-none focus-visible:ring-4 focus-visible:ring-accent"
                >
                  {/* Layer 1 (bottom): After image — always fully visible */}
                  <div
                    className="pointer-events-none absolute inset-0 flex h-full w-full transition-transform duration-500 ease-in-out"
                  ...
                  >
                    {cases.map((c, i) => (
                      <img
                        ...
                        decoding="sync"
                        draggable={false}
                        className="pointer-events-none h-full w-full shrink-0 object-cover object-center select-none"
                      />
                    ))}
                  </div>
              ...
              <div className="mt-4 flex flex-col items-center justify-between gap-3 px-2 text-white/70 sm:flex-row sm:px-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  {/* Dot indicators */}
                  <div className="flex items-center gap-1.5" aria-label="Case navigation">
                    {cases.map((_, i) => (
                      <button
                        key={i}
                        onClick={() => goTo(i)}
        ...
        <Reveal delay={240} className="mt-10 text-center">
          <div className="flex flex-wrap items-center justify-center gap-4">
            ...
            >
              <Link to="/smile-gallery">View Smile Gallery</Link>
            </Button>
          </div>
